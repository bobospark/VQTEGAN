...
class GAN_Training():
    ...
    def train(self, opt):
        ...
        if not os.path.exists(f'/workspace/cache_gan_generators/{opt.model_name}'):
            os.mkdir(f'/workspace/cache_gan_generators/{opt.model_name}')
            
        # torch.save(generator.state_dict(), '/%s/cache_gan_generators/%s/%s_%s_%s_%s.pth'%('workspace',opt.model_name, opt.model_name, opt.few_shot_type, opt.dataset, opt.seed))
        with open('/workspace/cache_gan_generators/%s/%s_%s_%s_%s.pkl'%(opt.model_name, opt.model_name, opt.few_shot_type, opt.dataset, opt.seed), 'wb') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--active_log", type=bool , default = False, help="True | False")  # wandb 활성화 할지 말지
