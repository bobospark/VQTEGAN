...
from Model.VQTEGAN_Model import VQAugmentation
...
class train_VQAugmentation(nn.Module):
    def __init__(self, args):
        ...
        self.data = torch.load(f'{args.data_path}/contrastive.roberta')
    ...
    def train(self, args):
        ...
        if args.active_log:
            ...
            wandb.init(
                ...
                project=f"VQ-Augmentation-Generator-{args.model_name}",
                entity = "bobos_park",
                name = f'W/O_cosine_loss_Params_epochs_{args.n_epochs}_batch_{args.batch_size}_vqlr_{args.vq_lr}_num_code_{args.num_codebook_vectors}',
                reinit=True
        ...
        for epoch in range(args.n_epochs):
            ...
            # Save the model
            if not os.path.exists(f'/workspace/models'):
                os.mkdir(f'/workspace/models')

            with open( '/workspace/models/%s_%s_%s.pkl'%(args.model_name, args.vq_lr, args.num_codebook_vectors), 'wb') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--num-codebook-vectors", type = int , default = 100, help = "number of codebook vectors (default : 512)")
    ...
    parser.add_argument("--model-name", type=str, default='roberta-large', help="roberta-large | bert-large-uncased")
