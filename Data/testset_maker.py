...
def testset(args, test_results ):
    ...
    if args.augmentation == 'VQ_TEGAN':
        ...
        if not args.data_augmentation:
            save_test_result_specific = f'{args.save_conventional_test_result}(batch_{args.batch_size})/model_{args.model_name}_lr_{args.lr}_batch_{args.batch_size}_seed_{args.seed}_{args.sample}_sample'
    ...
    if args.augmentation == 'GAN':
        ...
        save_test_result_specific = f'{args.save_test_result}(batch_{args.batch_size})/{args.model_name}_lr_{args.lr}_batch_{args.batch_size}_seed_{args.seed}_gan'
        if not os.path.exists(save_test_result_specific):
            os.mkdir(save_test_result_specific)

    if args.augmentation == 'EDA':
        if not os.path.exists(f'{args.save_test_result}(num_aug_{args.num_aug}_batch_{args.batch_size})'):
            os.mkdir(f'{args.save_test_result}(num_aug_{args.num_aug}_batch_{args.batch_size})')
        
        save_test_result_specific = f'{args.save_test_result}(batch_{args.batch_size})/{args.model_name}_lr_{args.lr}_batch_{args.batch_size}_num_aug_{args.num_aug}_seed_{args.seed}_eda'
        if not os.path.exists(save_test_result_specific):
            os.mkdir(save_test_result_specific)
