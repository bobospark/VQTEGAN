...
def greedy_search(greedy_or_not, args, model_name, model):
    num_of_training = 0
    num_codebook_vectors = [10, 20, 100 ]  # 1024,   , 200,
    vq_lrs = [ 2e-05, 5e-05]  #  ,      1e-5  , 2e-05, 5e-05
        ...
        if greedy_or_not:
                for vq_lr in vq_lrs:
                    for num_codebook_vector in num_codebook_vectors:
                        ...
                        args.vq_lr = vq_lr
                        print(num_of_training, '/', len(num_codebook_vectors)*len(vq_lrs))  # len(datasets)*
                        model(args)
    ...
    if model_name == 'GAN':
        ...
        model_names = ['roberta-large', 'bert-large']  # , 
    ...
    if model_name == 'GAN-Finetuning':
        lrs = [1e-5, 2e-5]  # 
        datasets = ['sst-2', 'cola', 'mrpc', 'qnli', 'rte', 'wnli', 'qqp', 'mnli', 'mnli-mm', ]  #  
        seeds = [13, 21, 42, 87, 100]  # 
        batch_sizes = [4,8]  #
        model_names = ['roberta-large', 'bert-large']  # , 
        if greedy_or_not:
